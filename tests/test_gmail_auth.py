...
class TestGmailAuth:
    ...
    def test_environment_setup(self):
        ...
        try:
            from meeting_minutes.config.app_config import (
                GOOGLE_OAUTH,
    ...
    def test_credentials_file_validation(self, mock_credentials_data):
        ...
        try:
            ...
            with patch(
                "builtins.open", mock_open(read_data=json.dumps(mock_credentials_data))
            ):
                with patch("os.path.exists", return_value=True):
                    ...
                    assert any(
                        expected_uri in uri or uri.endswith("/oauth/callback")
    ...
    def test_token_validation(self, mock_token_data):
        ...
        try:
            ...
            with patch(
                "builtins.open", mock_open(read_data=json.dumps(mock_token_data))
            ):
                with patch("os.path.exists", return_value=True):
                    with patch.object(
